...
def levenshtein_distance(original_sring="", new_string="", mismatch_cost=1, delete_insert_cost=1, ):
    dist_matrix = [[None] * (len(new_string) + 1) for _ in range(len(original_sring) + 1)]
    for i in range(len(original_sring) + 1):
        for j in range(len(new_string) + 1):
            if i == 0:
                ...
                current_dist = 0 if original_sring[i - 1] == new_string[j - 1] else mismatch_cost
    ...
    best_dist = dist_matrix[len(original_sring)][len(new_string)]
    edit_path_result = edit_path(dist_matrix, original_sring, new_string)
